...
class Loader {
    ...
    loadManifests(actor, file_list) {
        ...
        load_queue.loadManifest(cjs); // start the download

        // Manifest files : Those files define the spritesheets and such
    ...
    loadSpriteSheets(actor, spritesheets) {
        let to_load = []; // list all files to load
        for (const file of spritesheets) {
            if (file.id && !(file.id in window.images)) {
        ...
        if (actor.weaponTextures.length > 0) {
            ...
            loading_weapons[actor.weaponTextures[0]] = is_melee
                ? ["weapon_l", "weapon_r"]
                : "weapon";
            // init aux weapons
            let w2a = false;
            ...
            // init other textures
            for (let i = 1; i < actor.weaponTextures.length; ++i) {
                if (is_melee) {
                    if (actor.weaponTextures[i] in loading_weapons) {
                        weapon_dupe_table["weapon_version_" + i + "_0"] = loading_weapons[actor.weaponTextures[i]][0];
                        weapon_dupe_table["weapon_version_" + i + "_1"] = loading_weapons[actor.weaponTextures[i]][1];
                    } else {
                        to_load.push({
                            "loadTimeout": 60000,
                            "cache": true,
                            "src": Game.imgUri + "/sp/cjs/" + actor.weaponTextures[i] + "_1.png",
                        ...
                        to_load.push({
                            "loadTimeout": 60000,
                            "cache": true,
                            "src": Game.imgUri + "/sp/cjs/" + actor.weaponTextures[i] + "_2.png",
                            "id": "weapon_version_r_" + i,
                            "type": "image",
                            "ext": "png",
                            "path": ""
                        });
                        loading_weapons[actor.weaponTextures[i]] = ["weapon_version_l_" + i, "weapon_version_r_" + i];
                    }
                } else {
                    if (actor.weaponTextures[i] in loading_weapons) {
                        weapon_dupe_table["weapon_version_" + i] = loading_weapons[player.weaponTextures[i]];
                    } else {
                        to_load.push({
                            "loadTimeout": 60000,
                            "cache": true,
                            "src": Game.imgUri + "/sp/cjs/" + actor.weaponTextures[i] + ".png",
                            "id": "weapon_version_" + i,
                            "type": "image",
                            "ext": "png",
                            "path": ""
                        });
                        loading_weapons[actor.weaponTextures[i]] = "weapon_version_" + i;
                    }
                }
                // auxiliary
                if (w2a) weapon_dupe_table["weapon_version_2a_" + i] = "weapon2a";
                if (w2b) weapon_dupe_table["weapon_version_2b_" + i] = "weapon2b";
                if (manatura) weapon_dupe_table["familiar_version_" + i] = "familiar";
                if (shield) weapon_dupe_table["shield_version_" + i] = "shield";
            }
        }
        ...
        load_queue.on("error", function (event) {
            if (event && event.srcElement) console.error("Failed to download spritesheet", event.srcElement.src);
