...
@Transactional
@Component
public class HexachromaticLightDarkLogic extends DefaultEnemyMoveLogic {


    private final String gid = "7300833";

    protected HexachromaticLightDarkLogic(EnemyMoveLogicDependencies dependencies) {
        super(dependencies);
        registerLogics();
    }
    ...
    public MoveLogicResult triggerOmen(MoveLogicRequest request) {
        ...
        if (triggerOmen == null && self.getChargeGauge() >= self.getMaxChargeGauge()) {
            // 암룡: B 광룡: C
            MoveType standbyMoveType = StatusUtil.getEffectByName(self, "암룡의 기운")
                    .map(earthEffect -> StatusUtil.getEffectByName(self, "광룡의 기운")
                            .map(waterEffect -> {
    ...
    protected MoveLogicResult firstSupportAbility(MoveLogicRequest request) {
        ...
        StatusUtil.getEffectByName(self, "토룡의 기운").ifPresent(darkEffect -> {
        ...
        StatusUtil.getEffectByName(self, "수룡의 기운").ifPresent(lightEffect -> {
        ...
        StatusUtil.getEffectByName(self, "푸른 쐐기").ifPresent(goldWedge -> {
            setStatusLogic.removeStatusEffect(self, goldWedge); // 효과 보여주진 않음 (다른 쐐기 걸리는 효과로 갈음)
        });

        List<BaseStatusEffect> toApplyEffects = new ArrayList<>(ability.getBaseMove().getBaseStatusEffects()); // 황금 쐐기 효과
        ...
        DefaultMoveLogicResult defaultResult = defaultAbility(DefaultMoveRequest.withSelectedBaseStatusEffects(ability, toApplyEffects)); // 황금 쐐기 + (용의 기운) 효과
    ...
    // [TURN_END] 적에게 랜덤 약화효과 / 랜덤 용의 기운 레벨 1 상승
    ...
    protected MoveLogicResult secondSupportAbility(MoveLogicRequest request) {
        ...
        log.info("randomBuffEffects: {}", randomBuffEffects);
    ...
    protected MoveLogicResult thirdSupportAbility(MoveLogicRequest request) {
        ...
        if (checkCondition.isEnemyBreak(request.getOtherResult(), MoveType.STANDBY_A)) {
            ...
            StatusUtil.getEffectByName(self, "광룡의 기운").ifPresent(currentEffects::add);
            StatusUtil.getEffectByName(self, "암룡의 기운").ifPresent(currentEffects::add);
            ...
            // 페디엘 CT 해제
            return StatusUtil.getEffectByName(self, "암룡의 기운").map(statusEffect ->
            ...
            // 루오 CT 해제
            return StatusUtil.getEffectByName(self, "광룡의 기운").map(statusEffect ->
                            resultMapper.toResult(ResultMapperRequest.of(
                                    request.getMove(), setStatusLogic.subtractStatusEffectLevel(self, 3, statusEffect))
                            ))
                    .orElse(resultMapper.emptyResult());
    ...
    protected MoveLogicResult fourthSupportAbility(MoveLogicRequest request) {
        ...
        self.updateLatestTriggeredHp(100 - 20 * (formOrderByHp - 1) + 1); // formOrderByHp = 4, latestTriggeredHp = 41
