...
public class BattleContext {
    ...
    private BattleActor mainCharacter;
    private BattleActor enemy;

    private List<BattleActor> frontCharacters; // valid front
    private List<BattleActor> subCharacters; // valid sub

    private List<BattleActor> allActors; // with non-valid front and back
    /**
    ...
    public void init(Member member, Long mainCharacterId) {
        this.member = member;
        this.allActors = member.getBattleActors().stream()
                .sorted(Comparator.comparing(BattleActor::getCurrentOrder))
                .toList();
     ...
     * @return
     */
    public BattleActor getLeaderCharacter() {
        return allActors.stream().filter(actor -> actor.getActor().isLeaderCharacter()).findFirst().orElse(null);
    }
     ...
     */
    public void frontCharacterDead(BattleActor deadActor, BattleActor firstSubCharacter) {
